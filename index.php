	<?php /* Template Name: Index */ ?>
	<?php get_header(); ?>

	<div>
		<section class="sec1">
			<div class="container">
				<div class="row">
					<div class="col-md-6 col-sm-12 sec1Box">
						<div>
							<h2>
								<span class="h2Black">Enjoy ur</span>
								<br />
								<span class="h2Blue">Diet Plan</span>
							</h2>
							<p>Choose your delicious meals and plan your schedule to maintain your body</p>
							<div class="sec1Btn">
								<button class="blueBtn" style="margin-right: 15px;">Subscribe now</button>
								<button class="orangeBtn">Download App</button>
							</div>
						</div>
					</div>
					<div class="col-md-6 col-sm-12 homeBnrImg">
						<img src="http://159.223.17.72/wp-content/uploads/2023/05/homeBanner1.png" />
						<img src="http://159.223.17.72/wp-content/uploads/2023/05/homeBanner2.png" style="margin-left: -100px;" />
					</div>
				</div>
				<h3 class="bigWelcome">Welcome</h3>
			</div>
		</section>
		<section class="sec2">
			<div class="container">
				<div class="row justify-content-around mt-4">
					<div class="col-md-4 serviceBox">
						<div class="row">
							<div class="col-md-6">
								<img class="serBoxImg" src="http://159.223.17.72/wp-content/uploads/2023/05/Image-6@2x.png" height="375px" />
								<button class="orangeBtn">Subscribe now</button>
							</div>
							<div class="col-md-6 serDetailList">
								<h4>Lifestyle</h4>

								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="SDLpointText">
									<h6>149.00 KWD</h6>
									<p class="pText">Per Day</p>
								</div>
							</div>
						</div>
					</div>
					<div class="col-md-4 serviceBox">
						<div class="row">
							<div class="col-md-6">
								<img class="serBoxImg" src="http://159.223.17.72/wp-content/uploads/2023/05/Image-4@2x.png" height="375px" />
								<button class="orangeBtn">Subscribe now</button>
							</div>
							<div class="col-md-6 serDetailList">
								<h4>Muscle+</h4>

								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="SDLpointText">
									<h6>149.00 KWD</h6>
									<p class="pText">Per Day</p>
								</div>
							</div>
						</div>
					</div>
					<div class="col-md-4 serviceBox">
						<div class="row">
							<div class="col-md-6">
								<img class="serBoxImg" src="http://159.223.17.72/wp-content/uploads/2023/05/Image-11@2x.png" height="375px" />
								<button class="orangeBtn">Subscribe now</button>
							</div>
							<div class="col-md-6 serDetailList">
								<h4>TheDiet</h4>

								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="d-flex">
									<img src="http://159.223.17.72/wp-content/uploads/2023/05/food.png" height="25px" />
									<div class="SDLpointText">
										<p class="hText">x3 Meals</p>
										<p class="pText">Per Day</p>
									</div>
								</div>
								<div class="SDLpointText">
									<h6>149.00 KWD</h6>
									<p class="pText">Per Day</p>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</section>
		<section class="sec3 mt-5">
			<div class="container sec3Container">
				<div class="row">
					<div class="col-md-6" style="align-self: center;">
						<h3><span class="whiteText">Don't Know</span>

							<span class="orangeText">which plan?</span>
						</h3>
						<p>We'll guide you to choose the best plan to fit your objectives and dietitian goals based on your body and calorie calculator.</p>
						<div class="sec3BtnGrp">
							<button class="whiteBtn" style="margin-right: 15px;">Find my Plan</button>
							<button class="orangeBtn">Book an apointment</button>
						</div>
					</div>
					<div class="col-md-6 " style="align-self: end;padding-top:30px;">
						<img src="http://159.223.17.72/wp-content/uploads/2023/05/Image-16-e1684940073250.png" height="450px" class="drImg" />
					</div>
				</div>
			</div>
		</section>
		<section class="sec4 mt-5">
			<div class="container">
				<div class="row">
					<div class="col-md-6 orangeBox">
						<div>
							<h3>Taste our <br /> Menu Now</h3>
							<p>View our set menu for the month & taste our delicious meals delivered to your step-door, Fresh & Clean</p>
							<button class="whiteOrnBtn">Order Now</button>
						</div>
						<img src="http://159.223.17.72/wp-content/uploads/2023/05/Image-20.png" height="450px" />
					</div>
					<div class="col-md-6 h3Orng">
						<h3>THE BEST TO ENJOY HEALTH LIFESTYLE</h3>
					</div>
				</div>
			</div>
		</section>
		<section class="sec5 mt-5">
			<div class="container sec5Bnr">
				<img src="http://159.223.17.72/wp-content/uploads/2023/05/Mask-Group-3.jpg" width="100%" />
				<div class="row">
					<div class="col-md-5 offset-1">
						<h3>YOU ARE PART OF OUR STORY TIMELINE</h3>
					</div>
				</div>
			</div>
		</section>
		<section class="sec6 mt-5">
			<div class="container">
				<h3>Publishes<br /> & Blogs</h3>
				<div class="row PBContent">
					<div class="col-md-4" style="align-self: flex-end;">
						<div class="sec6Box">
							<p>We publish articles related to the field of nutrition and healthy diets to support those interested in enjoying a healthy lifestyle with a variety of options and more platforms available on the explore.</p>
							<button class="blueBtn">Explore more</button>
						</div>
					</div>
					<div class="col-md-8">
						<div class="row justify-content-around sec6ImgBox">
							<div class="col-md-3 sec6SerBox" style="background-image: url('http://159.223.17.72/wp-content/uploads/2023/05/Rectangle-81.png');">
								<p class="smallP">Thursday 10, May 2023</p>
								<p class="bigP">How can a diet plan improve your health and build you muscles?</p>
								<button>Read more</button>
							</div>
							<div class="col-md-3 sec6SerBox" style="background-image: url('http://159.223.17.72/wp-content/uploads/2023/05/Rectangle-85.png');">
								<p class="smallP">Thursday 10, May 2023</p>
								<p class="bigP">How can a diet plan improve your health and build you muscles?</p>
								<button>Read more</button>
							</div>
							<div class="col-md-3 sec6SerBox" style="background-image: url('http://159.223.17.72/wp-content/uploads/2023/05/Rectangle-89.png');">
								<p class="smallP">Thursday 10, May 2023</p>
								<p class="bigP">How can a diet plan improve your health and build you muscles?</p>
								<button>Read more</button>
							</div>

						</div>
					</div>

				</div>
			</div>
		</section>
	</div>


	<?php get_footer(); ?>